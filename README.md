# kai-pdf
This PDF viewer for vue.js or nuxt.js to display just pdf pages so it can be customized by your own layout.
...
## Fixed problem
* Print pdf garbled problem
* PDFJS version problem  
...
[kai-pdf demo on jsfiddle](https://jsfiddle.net/fossfiddler/5k4ptmqg/145/)
...
##### Example - vue.js display multiple pages of the same pdf document
```vue
<template>
	<div>
		<pdf
...
##### Example - nuxt.js display multiple pages of the same pdf document
```vue
<template>
	<div>
		<pdf
...
export default {
	...
	methdos: {
      ...
      this.pdfLoadTask();
		...
		pdfLoadTask() {
      if (process.browser) {
        this.vuePdf = require("kai-pdf");
        const propsLink = this.fileURL;
        this.finalPdfURL = this.vuePdf.default.createLoadingTask(propsLink);
        this.finalPdfURL.promise.then((pdf) => {
          this.numPages = pdf.numPages;
        });
      }
    },
